...
> Canonical sources: [domain/02-process-flow.md](domain/02-process-flow.md)
> (stages, gates), [design/approval-workflow.md](design/approval-workflow.md)
> (the chain), [design/roles-permissions.md](design/roles-permissions.md)
> (who can do what),
> [design/frontend/13-page-role-matrix.md](design/frontend/13-page-role-matrix.md)
> (pages).
...
| 6 | **Division Head** | `/tasks` → **Issuance tab** (`…/nol-issuance`) | Sets **approved terms** (may differ from what was requested), any **Kontrak Bersyarat** conditions, and the NOL validity period | **Setuju** → issues NOL · **Tidak Layak** → issues RL — both terminal | last Reviewer |
...
| At Area Head | **Nobody** — read-only; changes only possible if Area Head sends it back via `Revisi`, which returns it to Sales Area as `DRAFT` |
