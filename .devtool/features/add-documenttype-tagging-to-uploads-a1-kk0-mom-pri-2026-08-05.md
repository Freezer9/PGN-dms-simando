...
## Acceptance Criteria
- [ ] `DocumentType` enum/lookup added, and `SubmissionRecord` gets a `DocumentType` field
- [ ] Multiple documents per stage supported (remove the implicit one-per-stage constraint in `DocumentUpload.razor:35`'s filtering logic)
- [ ] Existing upload endpoint extended to accept a document type on upload
