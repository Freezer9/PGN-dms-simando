...
## Acceptance Criteria
- [x] `GET /api/subscriptions/{id}/contacts`, `POST`, `PUT /{contactId}`, `DELETE /{contactId}` added to `SubscriptionsController.cs`
- [x] Authorization matches existing subscription-edit role (`SalesArea`)
- [x] Activity log entry written on contact add/edit/remove, consistent with `ActivityLogs` usage elsewhere
