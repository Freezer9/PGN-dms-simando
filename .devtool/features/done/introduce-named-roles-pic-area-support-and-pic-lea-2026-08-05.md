...
## Acceptance Criteria
- [x] `PicAreaSupport` and `PicLeaderAreaSupport` roles added to `SimandoRoles.cs` (both `Api` and `Frontend` copies)
- [x] `AreaHead` granted an actual authorization policy for its approval step
- [x] Existing `Reviewer`/`DivisionHead` roles reconciled with the new named roles (decide whether they're replaced or kept as a generic fallback)
